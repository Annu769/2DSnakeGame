...
public class Snake : MonoBehaviour
{
    ...
    [Header("Score Info")]
    [SerializeField] ScoreUI score_Controller;
    [SerializeField] int scoreValue = 10;
    int currentScoreValue;

    [Header("Game Over Info")]
    [SerializeField] GameMenuManager gameMenu_Manager;
    ...
    void SetPowerUp(SpecialPower.PowerUpsType type)
    {
        switch (type)
        {
            case SpecialPower.PowerUpsType.Speed:
                speedController = 1.20f;
                break;

            case SpecialPower.PowerUpsType.Slow:
                speedController = 1f;
                break;

            case SpecialPower.PowerUpsType.ScoreBooster:
                scoreValue *= 2;
                break;
        }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.GetComponent<SpecialPower>() != null)
        {
            SetPowerUp(collision.GetComponent<SpecialPower>().GetPowerUpsType());
        }
